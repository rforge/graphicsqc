...
\begin{abstract}
  ...
  (when available). Lastly, information about these comparisons can be
  transformed into an HTML report.
\end{abstract}
\tableofcontents
...
software \citep{boudup04}. The \R{} language and environment for
statistical computing and graphics \citep{R,ihagen96} has
tools which ensure that \R{} code will run without catastrophic
...
of plots is performed and how some extra features of the package
provide for this.
...
\R{} \citep{hor02}. When writing packages, there is an \texttt{R CMD
...
if they exist. Lastly, it will evaluate code in any package vignettes
if they exist, to test that the code will not crash or produce any
...
\url{http://CRAN.R-project.org/}) and R-Forge
(\url{http://R-forge.R-project.org/}) websites, which both offer
nightly checks and builds of the packages they host.
\end{sloppypar}
...
named \mbox{\graphicsqc{}} written by \citet{murhor03}. It
...
returned, with no other information. For the files in the \texttt{png}
...
\verb|plotExpr()|, \verb|plotFile()| and \verb|plotFunction()|. These
evaluate and plot arbitrary expressions, code within files, and
example code from functions respectively. They also record `logs' of
the information about the plots they produce both as \R{} objects and
in XML files, including any warnings or errors that may have
occurred. These functions are used for the creation of both control
and test output and are further described in Chapter~\ref{chap:plot}.
...
function. The \verb|compare()| function will compare two sets of
plots, for example two sets produced by \verb|plotFunction()|. It
can compare using the \R{} objects, or paths to the corresponding log
files, or a mixture of these. It uses the GNU \texttt{diff} utility to
do the comparisons, which can be assumed on all of \R{}'s supported
platforms \citep{Rext08}. It compares the plots and any warnings or
errors for differences and then records the result of the
...
using the \verb|lapply()| function. There are two advantages for
this. First, the computation is vectorised, which \R{} has been
optimised for. Secondly, it ensures that \verb|evalPlotCode()| gets
...
% I'm sticking with unfeasible. Cambridge dictionary suggests
...
functions in \R{}, and the ability of users to create their own on the
fly, it is unfeasible to determine whether code will produce plots
without first evaluating it. So initially \texttt{evalPlotCode()} will
open the appropriate graphics device, evaluate the code, and then
close the device. If no plot is produced, this will leave a `blank'
image in the chosen file format. These `blank' images are not always
blank in the sense that some information is written to the file. This
information differs by file format and so the resulting sizes of files
will be different. These plots are not of interest when comparing plots
as they do not represent a plot produced by \R{}. To deal with this,
...
that are produced are then compared to these model `blank' images, and
removed if they are the same size, that is, completely blank. However,
warnings and errors are still recorded in these situations.
...
resulting log is given the class \texttt{qcPlotExprResult}.
...
respectively. Thus the resulting logs contain lists of
...
packages as a \texttt{qcPlotPackageResult} would consist of multiple
...
resulting class diagram is shown in Figure~\ref{fig:plotClass}

\begin{figure}
  \centering
    \includegraphics[height=8cm,keepaspectratio=true]{plotClass}
  \caption{Class diagram for `plot' objects}
  \label{fig:plotClass}
\end{figure}

How the \R{} objects are recorded as logs is described in the next
section.
...
The output from this example is not shown for brevity's sake. A
dynamically generated report better shows this information and is
described in Chapter~\ref{chap:report}.
...
\citep{w3cxml}. It is extensible due to the ability for users to
define their own elements. It is an `open standard' which agrees with
the open nature of \R{}. Some of the main advantages for using XML are
its extensibility for user-defined elements, it is self-documenting,
...
were created; one in PDF and one in PNG, with neither having
...
log file to refer to all the produced logs.
\end{sloppypar}

It is worth noting that the \R{} representations of
\texttt{qcPlotFileResult} and \texttt{qcPlotFunResult} objects contain
complete information, that is, all of the information contained in the
sub-\texttt{qcPlotExprResult} objects as well.
...
used to produce an image highlighting the differences between two
images. An example of this is given in
...
in order for \texttt{compare()} to work as primarily the differences
are detected using \texttt{diff}.

The \texttt{compare()} function handles comparisons between all
supported sets of expressions, for example between a
\texttt{qcPlotExprResult} and a \texttt{qcPlotExprResult} log, or
between a \texttt{qcPlotFileResult} and a \texttt{qcPlotFileResult}
log. The images that highlight differences are created by default if
ImageMagick is installed and the plots are different. The
\texttt{erase} argument to \texttt{compare()} specifies options for
removing test output after performing the comparison. If
\texttt{erase} is set to ``files'', that is, delete all the plots in
the test group (leaving only the log files), or set to ``all'', that
is, delete everything in the test group, then the plots
highlighting the differences will not be produced. This is currently
the only effect of the \texttt{erase} option as it does not yet remove
any files.
...
\subsection{Binary vs. Text-based Formats}
\label{sec:textvsbitmap}
Image formats are either binary or text-based. The contents of images
stored in a binary format are generally not readable for humans,
whereas text-based formats are. When images are stored in a binary
format, the files are relatively large and limited to the resolution
used at the time of saving. \citet{murhor03} noted that the version of
the third-party software used to view the image can produce slightly
different output, and even differences in hardware setup (for example
the platform being used). Currently the only supported binary format
is \texttt{png} (Portable Network Graphics).

Text-based formats tend to produce relatively small files at very high
resolutions, and are generally more platform-independent. A slightly
different problem faced with these formats is that it is possible to
alter the file and have no visible result on the final image. For
example a change in the driver used to create the image can change the
internal structure of the file, but still produce the same image. The
currently supported text-based formats are \texttt{ps} (PostScript)
and \texttt{pdf} (Portable Document Format).

Due to these differences between file formats, it is beneficial to use
multiple formats. This is so each format can be tested against itself
which helps identify whether changes in graphics output are due to
changes in the code used to produce them, or whether there is a
problem or change for a specific format.
...
knowledge of the format to interpret. Due to nature of binary formats,
...
differences, but this is not created for binary formats.
...
\section{Auto-detection of logs}
\begin{sloppypar}
...
as the former will contain several of the latter. Objects of class
\texttt{qcPlotPackageResult} would be considered most important,
however these are not yet implemented. The way for specifying these
can be mixed-and-matched, for example the \texttt{test} group could be
an \R{} object while the \texttt{control} group could be specified as
the path to the file to compare. This works as long as the resulting
classes for \texttt{test} and \texttt{control} are the same, for
example a \texttt{qcPlotFunResult} and a \texttt{qcPlotFunResult}. If
they are not, an error is given. In the case where the log file is an
ambiguous choice, an error is also given, for example if the same
folder was given for the \texttt{test} group and for the
\texttt{control} group, even if two separate files are detected, it is
unclear which should be the test group and which should be the
control.  An example of this auto-detection in use is given in
Section~\ref{sec:gridCompare}.
\end{sloppypar}
...
comparison. The code has been designed so that it is easy for
developers to add new formats. The name of the function called to
compare two plots is based on the name of the format that is being
compared. For example if the current format was \texttt{png}, then the
function that gets called to perform the comparison is
\texttt{comparePNG()}. That is, the word \texttt{compare} with the
format being compared in uppercase appended on, \texttt{PNG}. So if a
new format were to be supported by \graphicsqc{}, the only change
necessary for comparing the format would be a new, appropriately named
function.

The only changes necessary to include a new format for plotting are to
add the mapping of file format to device in \texttt{evalPlotCode()},
and to include the format in a list of \texttt{validFiletypes} in
\texttt{getValidFiletypes()}.

\chapter{Generating Reports}
\label{chap:report}
Once plots and comparisons have been performed, the results are stored
in XML files. At this point, users of the package may wish to take the
process entirely out of \R{} or do anything they like with them. One
option is to dynamically generate a report based on the plots or
comparisons. As the data are stored in XML files, there is a language
defined that is useful for transforming XML documents into other types
of documents. This language, XSL, and how it is used to transform the
log files into HTML reports is described in the following sections.

\section{HTML Reports}
Depending on the plots or comparisons performed, there is generally a
large amount of information to report on. The nature of how the log
files are stored reflects well with the natural nature of HTML where
different pages are connected to each other via links. For example
\texttt{qcPlotFunResult} logs typically refer to other
\texttt{qcPlotExprResult} logs. They also refer to different plots,
which can be linked to and displayed within an HTML page. There is a
very natural way of transforming the log files from XML into HTML
which is described in Section~\ref{sec:xml2html}. Web browsers are
very widespread, so most users will be able to view HTML pages.
...
other formats \citep{w3cxsl}. This is also sometimes referred to as XSLT. In XSL,
...
first argument, \texttt{qcResult}. The \texttt{qcResult} argument can
be the result of any plot or comparison generated by \graphicsqc{}. If
the function is given a `more important' object, for example a
\texttt{qcPlotFunResult} object, it will produce an HTML report for
the current object, as well as all objects that the current object
refers to. This is because the page that is generated links to the
sub-objects that are referred to in case the user wishes to see more
detail. That is, for every XML log file that is referred to, a
corresponding HTML file will be created with the same name, but with
the \texttt{.html} extension. The \texttt{writeReport()} function is
flexible in specifying which object to report on. It can accept either
the \R{} object, the path to the log file, or even auto-detect from a
directory. If a directory is given, first any comparison log files are
searched for, followed by log files created by plots, in decreasing
order of `importance'. Each object that is reported on has the
appropriate stylesheet applied, however the default stylesheets can be
overwritten by specifying the path to the desired stylesheets by the
\texttt{xslStyleSheets} argument.

An example of the report generated from the `\texttt{firstExample}' in
Section~\ref{sec:plotExpr} is given in
...
binary formats (PNG), but not the text-based formats (PostScript and
PDF). The magnitude and scope of the effects were also not known. The
\graphicsqc{} package was used to compare revision 44417 of \R{}
against the previous revision, 44416, to establish what effects this
change might have had on the \textbf{grid} package.
...
grid44417 <- plotFunction(ls("package:grid"),
                          ...
                          path="~/tests/R44417")
@
With a similar command used in revision 44416, given below.
<<plotGrid2, eval=FALSE>>=
grid44416 <- plotFunction(ls("package:grid"),
                          filetype = c("pdf", "ps", "png"),
                          path="~/tests/R44416")
@
...
group, prior to the change, and revision 44417 to be the test
group. The comparison will be performed under revision 44417, however
the revision under which the comparison is performed will not make a
...
\end{verbatim}
which gives the path to the HTML report. Note that it has the prefix
``\texttt{absolute.size}'' because that is the first function in the
list of functions that were compared.
\end{sloppypar}
...
however had differences in all three formats being tested, which was a
completely unexpected result, illustrating the usefulness of the
\graphicsqc{} package.
...
The results of this example, including the generated report can be
downloaded as a tarball from:\\
\url{http://graphicsQC.R-Forge.R-Project.org/gridExample.tar.gz}

\begin{figure}
  \centering
    \includegraphics[width=12cm, keepaspectratio=TRUE]{reportGrid1}
  \caption{Information section of the \textbf{grid} comparison report.}
...
\begin{figure}
  \centering
    ...
    \caption{Functions with different plots in the \textbf{grid}
      comparison.}
...
\begin{figure}
  \centering
    ...
    \caption{Functions with identical plots in the \textbf{grid}
      comparison.}
...
\begin{figure}
  \centering
    ...
    \caption{Functions with no plots produced in the \textbf{grid}
      comparison.}
...
\begin{figure}
  \centering
    ...
    \caption{Differences in warnings, errors, and any unpaired plots
      in the \textbf{grid} comparison.}
...
\begin{figure}
  \centering
    ...
    \caption{Report of \texttt{arcCurvature()} comparison showing
      differences in the \texttt{png} format, but not in the
      \texttt{pdf} or \texttt{ps} formats. This page is a result of
      clicking on the ``arcCurvature+arcCurvature'' link in
      Figure~\ref{fig:reportGrid2}.}
...
comparisons. All of the information related to plots and comparisons
are stored in an open format, allowing for the ability to read the
...
for identifying both expected and unexpected changes in the graphics
generated by the \textbf{grid} package. The example also showed that
the package is only useful for detecting changes in graphics output,
but cannot identify \emph{why} changes occur.
...
%\appendix
\clearpage
%\renewcommand\thesection{Appendix \Alph{Documentation}}
\chapter*{{\huge Appendix A}\\[1cm]Documentation} \addcontentsline{toc}{chapter}{Appendix A: Documentation}
